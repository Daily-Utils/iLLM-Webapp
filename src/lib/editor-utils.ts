...
import { getDiscordConnectionUrl, getNotionConnection,getNotionDatabase,getSlackConnection } from "../server"
...
export const onDragStart = (
  ...
  export const onConnections = async (
    ...
    if (editorState.editor.selectedNode.data.title == 'Discord') {
      const connection:any = await getDiscordConnectionUrl(id)
    ...
    if (editorState.editor.selectedNode.data.title == 'Notion') {
      const connection :any = await getNotionConnection(id)
      if (connection && connection.data) {
    ...
    if (editorState.editor.selectedNode.data.title == 'Slack') {
      const connection:any = await getSlackConnection(id)
      if (connection && connection.data) {
